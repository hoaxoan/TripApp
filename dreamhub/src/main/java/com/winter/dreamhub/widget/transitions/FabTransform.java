...
public class FabTransform extends Transition {
    ...
    public FabTransform(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            if (!a.hasValue(R.styleable.FabTransform_fabTransformColor)
                    || !a.hasValue(R.styleable.FabTransform_fabTransformIcon)) {
                throw new IllegalArgumentException("Must provide both color & icon.");
            }
            color = a.getColor(R.styleable.FabTransform_fabTransformColor, Color.TRANSPARENT);
            icon = a.getResourceId(R.styleable.FabTransform_fabTransformIcon, 0);
            setPathMotion(new GravityArcMotion());
            if (getDuration() < 0) {
                setDuration(DEFAULT_DURATION);
            }
